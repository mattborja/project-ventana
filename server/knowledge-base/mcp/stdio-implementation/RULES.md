...
- Credential retrieval is done via `git credential fill` (synchronous child process). Do not suggest replacing this with PAT environment variables or other credential mechanisms unless GCM is confirmed unavailable in the target environment.
- The server intentionally uses Node.js built-in `https` rather than a third-party HTTP client to minimize dependencies.
...
- The credential returned by the git credential helper is used as a Basic auth token (`:<token>` base64-encoded). This is the correct format for PAT and bearer token authentication over HTTPS on most Git hosts.
