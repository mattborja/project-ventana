...
Credentials are sourced from the developer's git credential helper at runtime via `git credential fill`. The credential helper (Git Credential Manager, osxkeychain, libsecret, etc.) handles provider-specific HTTPS authentication flows and caches the resulting credential. No manual token management is required.
